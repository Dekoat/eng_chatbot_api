...
CORS(app, origins=['http://localhost', 'http://127.0.0.1'])  # CORS: localhost only
...
# Load model at startup (retry once on failure)
classifier = IntentClassifier()
try:
    classifier.load_model()
    logger.info("[OK] Model loaded successfully")
except FileNotFoundError:
    logger.warning("Model not found, attempting to train from scratch...")
    try:
        X, y = classifier.load_data()
        classifier.train(X, y, test_size=0)
        classifier.save_model()
        logger.info("[OK] Model trained and loaded successfully")
    except Exception as e:
        logger.error(f"❌ Could not load or train model: {e}")
...
def predict():
    ...
    try:
        ...
        if len(question) > MAX_QUESTION_LENGTH:
            return jsonify({
                'error': f'Question too long (max {MAX_QUESTION_LENGTH} chars)'
            }), 400
        
        # Use AI Model only (keyword rules removed for cleaner predictions)
...
def batch_predict():
    ...
    try:
        ...
        if len(questions) > MAX_BATCH_SIZE:
            return jsonify({
                'error': f'Too many questions (max {MAX_BATCH_SIZE})'
            }), 400
