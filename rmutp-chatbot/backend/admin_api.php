...
class AdminAPI {
    ...
    public function updateFAQ($token, $id, $data) {
        ...
        // ดึงข้อมูลเดิมก่อน → partial update จะไม่เขียนทับด้วยค่าว่าง
        $existing = $this->db->prepare("SELECT question, answer, category, keywords FROM faq WHERE id = ?");
        $existing->execute([$id]);
        $current = $existing->fetch(PDO::FETCH_ASSOC);
        
        if (!$current) {
            $this->sendError('FAQ not found', 404);
        }
        ...
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
            !empty($data['question']) ? $data['question'] : $current['question'],
            !empty($data['answer'])   ? $data['answer']   : $current['answer'],
            !empty($data['category']) ? $data['category'] : $current['category'],
            array_key_exists('keywords', $data) ? $data['keywords'] : $current['keywords'],
            $id
        ]);
