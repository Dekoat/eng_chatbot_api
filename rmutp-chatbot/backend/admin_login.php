...
class AdminAuth {
    ...
    public function login($username, $password) {
        try {
            // ตรวจสอบ username และ password (ดึงจาก .env)
            $validUsername = getenv('ADMIN_USERNAME') ?: 'admin';
            $validPassword = getenv('ADMIN_PASSWORD') ?: 'admin123';
    ...
    private function generateToken($username) {
        ...
        $signature = hash_hmac('sha256', $token, getenv('HMAC_SECRET') ?: 'rmutp_secret_key_2026');
    ...
    public function verifyToken($token) {
        try {
            ...
            // ตรวจสอบ signature (timing-safe)
            $expectedSignature = hash_hmac('sha256', $payload, getenv('HMAC_SECRET') ?: 'rmutp_secret_key_2026');
