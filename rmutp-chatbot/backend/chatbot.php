...
require_once __DIR__ . '/broad_topic_handler.php';
...
class AIHelper {
    ...
    private function checkCLI() {
        ...
        try {
            $testCmd = 'python ' . escapeshellarg($scriptPath) . ' ' . escapeshellarg('test') . ' 2>&1';
    ...
    private function predictIntentCLI($question) {
        ...
        // Escape คำถามสำหรับ command line (ใช้ escapeshellarg ป้องกัน command injection)
        $escapedQuestion = escapeshellarg($question);
        $cmd = 'python ' . escapeshellarg($scriptPath) . ' ' . $escapedQuestion . ' 2>&1';
...
class Chatbot {
    ...
    public function handleChat($sessionId, $message) {
        ...
        if ($this->ai->isEnabled()) {
            ...
            if ($prediction && $prediction['confidence'] > 0.7) {
                ...
                $notStaffKeywords = ChatbotConfig::$notStaffKeywords;
                $isReallyStaff = true;
                ...
                if ($aiIntent === 'ask_staff' && $isReallyStaff) {
                    // ตรวจก่อนว่า FAQ มีคำตอบ curated หรือไม่ (เช่น FAQ "รายชื่ออาจารย์สาขา...")
                    $curatedFaqId = $this->hasCuratedStaffFaq($message);
                    if ($curatedFaqId) {
                        error_log("Phase 1: AI says ask_staff but FAQ #{$curatedFaqId} has curated answer, letting FAQ handle");
                    } else {
        ...
        $staffKeywordsCheck = ChatbotConfig::$staffKeywordsCheck;
        ...
        $forceStaffFaqCategory = false;
        if ($isStaffQuestion) {
            $notStaffContext = ChatbotConfig::$notStaffContext;
            // เฉพาะคำถามเรื่อง ตำแหน่ง/บทบาท เท่านั้นที่บังคับ FAQ category 'staff'
            // คำอื่น (ที่ปรึกษา, เอกสาร, นิเทศ, งานวิจัย ฯลฯ) ให้ใช้ AI category ตามปกติ
            $staffRoleKeywords = ['ประธาน', 'ผู้รับผิดชอบ', 'ประจำหลักสูตร', 'ประจำสาขา', 'หัวหน้า'];
            foreach ($notStaffContext as $ctx) {
                if (mb_stripos($message, $ctx) !== false) {
                    $isStaffQuestion = false;
                    // บังคับ 'staff' category เฉพาะเมื่อถามเรื่องตำแหน่ง/บทบาท
                    if (in_array($ctx, $staffRoleKeywords)) {
                        $forceStaffFaqCategory = true;
                        error_log("Phase 2: Overridden! Found staffRole '$ctx' → skip staff search, force staff FAQ category");
                    } else {
                        error_log("Phase 2: Overridden! Found notStaffContext '$ctx' → skip staff search, use normal AI category");
                    }
                    break;
                }
            }
        ...
        if ($isStaffQuestion) {
            // ✅ ตรวจก่อนว่า FAQ มีคำตอบ curated สำหรับคำถาม staff นี้หรือไม่
            $curatedFaqId = $this->hasCuratedStaffFaq($message);
            if ($curatedFaqId) {
                error_log("Phase 2: FAQ #{$curatedFaqId} has curated staff answer, letting FAQ handle instead of staff table");
                // ไม่ return staff results → ปล่อยให้ Phase 3 FAQ search จัดการ
            } else {
                $staffResults = $this->searchStaff($message);
        ...
        $skipNewsKeywords = ChatbotConfig::$skipNewsKeywords;
        $skipNews = false;
        foreach ($skipNewsKeywords as $skw) {
            if (mb_stripos($message, $skw) !== false) {
                $skipNews = true;
                error_log("Phase 2.5: Skip news - found skipKeyword '$skw'");
                break;
            }
        }
        
        // ✅ Override: ถ้ามีคำที่บ่งบอกชัดเจนว่าถามข่าว → ยกเลิก skip (เช่น "ข่าวกิจกรรม", "กิจกรรมล่าสุด")
        $forceNewsIndicators = ['ข่าว', 'ล่าสุด', 'ข่าวสาร', 'ประชาสัมพันธ์'];
        if ($skipNews) {
            foreach ($forceNewsIndicators as $fnk) {
                if (mb_stripos($message, $fnk) !== false) {
                    $skipNews = false;
                    error_log("Phase 2.5: Skip override! Found news indicator '$fnk' → allow news search");
                    break;
                }
            }
        }
        ...
        // แต่ถ้ามีคำที่บ่งบอกชัดเจนว่าถามข่าว → ให้ค้นข่าวก่อน
        if ($aiIntent && $aiIntent !== 'ask_news' && $aiConfidence > 0.7) {
            $hasNewsIndicator = false;
            foreach ($forceNewsIndicators as $fnk) {
                if (mb_stripos($message, $fnk) !== false) {
                    $hasNewsIndicator = true;
                    error_log("Phase 2.5: AI says '$aiIntent' but found news indicator '$fnk' → allow news search");
                    break;
                }
            }
            if (!$hasNewsIndicator) {
        ...
        // ===== Phase 3.5: Broad Topic Detection =====
        // ตรวจจับคำถามกว้างๆ (สั้น ไม่ระบุสาขา) เช่น "ทุนการศึกษา", "ค่าเทอม", "ฝึกงาน"
        // แสดงภาพรวมหัวข้อ + รายการคำถามที่เกี่ยวข้อง แทนการสุ่มเลือก FAQ อันเดียว
        if (!$isStaffQuestion) {
            $broadTopic = BroadTopicHandler::detectBroadTopic($message);
            if ($broadTopic) {
                // ✅ ตรวจก่อน: ถ้า FAQ มี question ที่ตรงเป๊ะกับข้อความผู้ใช้ → ข้าม broad topic ให้ Phase 3 จัดการ
                // เช่น "รับสมัคร มทร.2569" ตรงกับ FAQ #93 → ไม่ต้องแสดงรายการเลือก
                // ตรวจแบบ exact: question = msg หรือ msg เป็นหนึ่งใน variant แยกด้วย |
                $exactCheckStmt = $this->db->prepare(
                    "SELECT id, question FROM faq WHERE is_active = 1 AND (
                        question = ? 
                        OR question LIKE CONCAT(?, '|%')
                        OR question LIKE CONCAT('%|', ?)
                        OR question LIKE CONCAT('%|', ?, '|%')
                    ) LIMIT 1"
                );
                $exactCheckStmt->execute([$message, $message, $message, $message]);
                $exactFaq = $exactCheckStmt->fetch(PDO::FETCH_ASSOC);
                
                if ($exactFaq) {
                    error_log("[BroadTopic] Skipping — exact FAQ match #{$exactFaq['id']} for '$message', letting Phase 3 handle it");
                    // ไม่ return → ปล่อยให้ Phase 3 FAQ Search จัดการ (จะได้คำตอบตรง ไม่ใช่รายการเลือก)
                } else {
                    $topicFAQs = BroadTopicHandler::searchRelatedFAQs($broadTopic, $this->db);
                    if (!empty($topicFAQs)) {
                        $topicResult = BroadTopicHandler::buildTopicOverview($broadTopic, $topicFAQs);
                        if ($topicResult) {
                            // buildTopicOverview return array ['answer' => ..., 'related_questions' => [...]]
                            $topicAnswer = $topicResult['answer'];
                            $topicRelated = $topicResult['related_questions'] ?? [];
                            $sources = BroadTopicHandler::buildSources(array_slice($topicFAQs, 0, 8));
                            $confidence = 90;
                            $responseTime = round((microtime(true) - $this->startTime) * 1000);
                            $this->logChat($sessionId, $message, $topicAnswer, $sources, $confidence, $responseTime);
                            
                            error_log("[Chatbot] Broad topic '{$broadTopic['id']}' detected for '$message' — showing topic overview (" . count($topicRelated) . " choices)");
                            ...
                            return [
                                ...
                                'related_questions' => $topicRelated,
                                'ai_used' => false
                            ];
                        }
                    }
        ...
        $aiCategory = null;
        if ($forceStaffFaqCategory) {
            // บังคับใช้ 'staff' category เพราะ notStaffContext ข้าม staff table
            $aiCategory = 'staff';
            error_log("Using forced 'staff' category for FAQ search (from notStaffContext override)");
        } elseif ($aiIntent && $aiConfidence > 0.4) { // ลด threshold ลงเพื่อใช้ AI มากขึ้น
            ...
            // ✅ Override: ถ้า AI บอก academic แต่คำถามมี "ทำไม" + "เรียน" → เปลี่ยนเป็น about
            // เพราะ "ทำไมควรเรียน/ทำไมต้องเรียน" = ถามเกี่ยวกับจุดเด่น ไม่ใช่เรื่องวิชาการ
            if ($aiCategory === 'academic' && mb_stripos($message, 'ทำไม') !== false && mb_stripos($message, 'เรียน') !== false) {
                $aiCategory = 'about';
                error_log("AI category override: academic → about (found 'ทำไม' + 'เรียน' = asking about reasons/advantages)");
            }
            
            // ✅ Override: ถ้า AI ไม่ได้บอก loan แต่คำถามมี keywords ที่ชัดเจนว่าเป็นเรื่องกู้ยืม/ค่าเทอม → บังคับ loan
            // เช่น "ฟอร์มกู้เกินหลักสูตร DSL" AI อาจบอก program เพราะมีคำว่า "หลักสูตร"
            if ($aiCategory !== 'loan') {
                $loanOverrideKeywords = ['กยศ', 'กรอ', 'DSL', 'กู้ยืม', 'กู้เกิน', 'ผู้กู้', 'เอกสารกู้', 'แบบฟอร์มกู้', 'ค่าเทอม', 'ค่าเล่าเรียน', 'เงินกู้', 'จิตอาสา กยศ'];
                foreach ($loanOverrideKeywords as $lk) {
                    if (mb_stripos($message, $lk) !== false) {
                        $oldCat = $aiCategory;
                        $aiCategory = 'loan';
                        error_log("AI category override: {$oldCat} → loan (found loan keyword '{$lk}' in message)");
                        break;
                    }
                }
            }
        ...
        // ✅ Override: ถ้ายังไม่ได้ตั้ง admission — ตรวจ keyword-based override แม้ AI ไม่ classify
        // ทำงานทั้งกรณี AI บอก category อื่น หรือ AI ไม่ตอบเลย
        if ($aiCategory !== 'admission' && $aiCategory !== 'loan' && $aiCategory !== 'staff') {
            $admissionOverrideKeywords = ['รับสมัคร', 'สมัครเรียน', 'รับจบ', 'รับผู้จบ', 'รับวุฒิ', 'จบวุฒิ', 'เทียบโอน', 'ย้ายมหาลัย', 'ย้ายมา', 'จบอะไร'];
            foreach ($admissionOverrideKeywords as $ak) {
                if (mb_stripos($message, $ak) !== false) {
                    $oldCat = $aiCategory ?? 'null';
                    $aiCategory = 'admission';
                    error_log("Keyword override: {$oldCat} → admission (found '{$ak}' in message)");
                    break;
                }
            }
            
            // เช็คเพิ่ม: "สมัคร" + ชื่อสาขา + ("จบ"|"ปวส"|"ปวช"|"ม.6") → admission
            if ($aiCategory !== 'admission' && mb_stripos($message, 'สมัคร') !== false) {
                $qualWords = ['จบ', 'ปวส', 'ปวช', 'ม.6', 'ม.3', 'วุฒิ'];
                $deptNames = ['วิศวะ', 'ไฟฟ้า', 'เครื่องกล', 'โยธา', 'อุตสาหการ', 'คอมพิวเตอร์', 'เมคคาทรอนิกส์', 'อัญมณี', 'เครื่องประดับ'];
                $hasQual = false;
                $hasDept = false;
                foreach ($qualWords as $qw) { if (mb_stripos($message, $qw) !== false) { $hasQual = true; break; } }
                foreach ($deptNames as $dn) { if (mb_stripos($message, $dn) !== false) { $hasDept = true; break; } }
                if ($hasQual && $hasDept) {
                    $oldCat = $aiCategory ?? 'null';
                    $aiCategory = 'admission';
                    error_log("Keyword override: {$oldCat} → admission (found 'สมัคร' + qualification + department)");
                }
            }
            
            // เช็คเพิ่ม: "ต่างชาติ" + ชื่อสาขา → admission
            if ($aiCategory !== 'admission' && mb_stripos($message, 'ต่างชาติ') !== false) {
                $deptNames = ['วิศวะ', 'ไฟฟ้า', 'เครื่องกล', 'โยธา', 'อุตสาหการ', 'คอมพิวเตอร์', 'เมคคาทรอนิกส์', 'อัญมณี', 'เครื่องประดับ', 'SIME', 'ยั่งยืน'];
                foreach ($deptNames as $dn) {
                    if (mb_stripos($message, $dn) !== false) {
                        $oldCat = $aiCategory ?? 'null';
                        $aiCategory = 'admission';
                        error_log("Keyword override: {$oldCat} → admission (found 'ต่างชาติ' + dept '{$dn}')");
                        break;
                    }
                }
            }
            
            // เช็คเพิ่ม: "จบ...เรียนได้/สมัคร" + ชื่อสาขา → admission
            if ($aiCategory !== 'admission' && preg_match('/จบ.{0,15}(เรียน|สมัคร)/u', $message)) {
                $deptNames2 = ['วิศวะ', 'คอมพิวเตอร์', 'ไฟฟ้า', 'เครื่องกล', 'โยธา', 'อุตสาหการ', 'เมคคาทรอนิกส์', 'อัญมณี'];
                foreach ($deptNames2 as $dn) {
                    if (mb_stripos($message, $dn) !== false) {
                        $oldCat = $aiCategory ?? 'null';
                        $aiCategory = 'admission';
                        error_log("Keyword override: {$oldCat} → admission (found 'จบ...เรียน/สมัคร' + dept '{$dn}')");
                        break;
                    }
                }
            }
        ...
        if (!empty($faqResults)) {
            ...
            $isGenericQuestion = $skipGenericCheck ? false : BroadTopicHandler::isGenericQuestion($message, $faqResults);
            
            if ($isGenericQuestion) {
                $departmentAnswers = BroadTopicHandler::getDepartmentSpecificAnswers($faqResults);
                
                if (count($departmentAnswers) >= 1 && count($faqResults) >= 3) {
                    $genericResult = BroadTopicHandler::buildGenericAnswer($departmentAnswers);
                    $answer = $genericResult['answer'];
                    $genericRelated = $genericResult['related_questions'] ?? [];
                    $sources = BroadTopicHandler::buildGenericSources($departmentAnswers);
                    $confidence = 85; // ความมั่นใจสูงเพราะให้เลือกชัดเจน
                    
                    error_log("[Chatbot] Showing generic answer with " . count($departmentAnswers) . " options (" . count($genericRelated) . " choices)");
                    ...
                    return [
                        ...
                        'confidence' => $confidence,
                        'response_time_ms' => $responseTime,
                        'related_questions' => $genericRelated,
                        'ai_used' => false
                    ];
                } else {
            ...
            if ($confidence < 35) {
                // ===== Last Resort: ลองค้นหาชื่ออาจารย์ก่อนตอบ "ไม่พบ" =====
                if (!$isStaffQuestion) {
                    $lastResortStaff = $this->searchStaffByName($message);
                    if (!empty($lastResortStaff)) {
                        return $this->buildStaffResponse($sessionId, $message, $lastResortStaff);
                    }
                }
                ...
                if ($isGenericQuestion && !empty($departmentSpecificAnswers)) {
                    ...
                    $deptLabels = [
                        ...
                        'sime' => 'วิศวกรรมการจัดการอุตสาหกรรมเพื่อความยั่งยืน',
                        'general' => 'ทั่วไป'
                    ];
            ...
            // No match found — ลองค้นหาชื่ออาจารย์ก่อนตอบ "ไม่พบ"
            if (!$isStaffQuestion) {
                $lastResortStaff = $this->searchStaffByName($message);
                if (!empty($lastResortStaff)) {
                    return $this->buildStaffResponse($sessionId, $message, $lastResortStaff);
    ...
    private function searchFAQBroad($query, $filterCategory = null) {
        ...
        // ใช้ config จาก ChatbotConfig (single source of truth)
        $intentPatterns = ChatbotConfig::$intentPatterns;
        ...
        // ✅ Override: ถ้า intent = admission แต่มีคำเกี่ยวกับ กยศ/กรอ/ทุน → เปลี่ยนเป็น scholarship
        // เพราะ "สมัคร กยศ" = scholarship ไม่ใช่ admission (สมัครเรียน)
        if ($queryIntent === 'admission') {
            $loanIndicators = ['กยศ', 'กรอ', 'กู้ยืม', 'ทุน', 'กองทุน', 'DSL', 'loan'];
            foreach ($loanIndicators as $li) {
                if (mb_stripos($query, $li) !== false) {
                    $queryIntent = 'scholarship';
                    error_log("Intent override: admission → scholarship (found loan indicator '$li')");
                    break;
                }
            }
        }
        ...
        // ใช้ config จาก ChatbotConfig (single source of truth)
        $departmentMap = ChatbotConfig::$departmentMap;
        
        // สำหรับ department detection ใช้เฉพาะส่วนแรก (ก่อน pipe) ของคำถาม
        // เพื่อไม่ให้ alternative text หลัง pipe ทำให้ dept filter ผิด
        $queryForDept = $query;
        if (strpos($query, '|') !== false) {
            $queryForDept = trim(explode('|', $query)[0]);
        }
        
        $detectedDepartment = null;
        foreach ($departmentMap as $keyword => $dept) {
            if (mb_stripos($queryForDept, $keyword) !== false) {
        ...
        $skipDeptKeywords = ChatbotConfig::$skipDeptKeywords;
        ...
        if ($filterCategory) {
            ...
            $relatedCategories = ChatbotConfig::$relatedCategories;
            ...
            // ⚠️ ไม่ใช้ OR f.department = 'general' เพราะจะดึง FAQ ที่ไม่เกี่ยวข้องเข้ามาด้วย
            // เช่น FAQ ห้องออกกำลังกาย (cat:contact, dept:general) จะ bypass filter ได้
            // แทนที่จะใช้ OR → ใช้ category filter อย่างเดียว (related categories ครอบคลุมแล้ว)
            $sql .= "AND f.category IN ($catPlaceholders) ";
            $params = array_merge($params, $categories);
            error_log("Filtering FAQ by categories: " . implode(', ', $categories));
        }
        
        // เพิ่ม department filter ถ้าตรวจพบชื่อสาขาชัดเจน (รวม general และ student_affairs เสมอ)
        if ($detectedDepartment) {
            $deptFilter = [$detectedDepartment, 'general', 'student_affairs'];
            // vocational dept → รวม parent engineering dept ด้วย
            if ($detectedDepartment === 'vocational_computer') {
                $deptFilter[] = 'computer_engineering';
            }
            // vocational dept → รวม vocational_computer ด้วย
            if ($detectedDepartment === 'vocational') {
                $deptFilter[] = 'vocational_computer';
            }
            $deptPlaceholders = implode(',', array_fill(0, count($deptFilter), '?'));
            $sql .= "AND f.department IN ($deptPlaceholders) ";
            $params = array_merge($params, $deptFilter);
            error_log("Filtering FAQ by department: " . implode(', ', $deptFilter));
        }
        ...
        // เพิ่มเงื่อนไข LIKE สำหรับแต่ละ keyword (max 10 คำ — เพิ่มจาก 5 เพื่อครอบคลุม commonWords ที่แยกจาก Thai)
        ...
        try {
            ...
            foreach ($results as &$row) {
                ...
                // แยก pipe-separated alternatives → ได้ array ของคำถามทั้งหมด
                $questionAlts = array_map('trim', explode('|', $question));
                $questionFirst = $questionAlts[0];
                
                // [+2000 pts] EXACT MATCH - ตรวจทุก alternative (ไม่ใช่แค่คำถามแรก)
                $exactMatched = false;
                $queryLower = mb_strtolower(trim($query));
                foreach ($questionAlts as $alt) {
                    if (mb_strtolower(trim($alt)) === $queryLower) {
                        $score += 2000;
                        $exactMatched = true;
                        error_log("EXACT MATCH FOUND! Q: $alt (+2000)");
                        break;
                    }
                }
                
                // [+1000 pts] IMPORTANT PHRASE BOOST - phrases ที่สำคัญต้องมาตรงกัน (cap max +2000)
                $importantPhrases = ChatbotConfig::$importantPhrases;
                $importantPhraseScore = 0;
                foreach ($importantPhrases as $phrase) {
                    if ($importantPhraseScore >= 2000) break; // Cap at +2000
                    if (mb_stripos($query, $phrase) !== false) {
                        ...
                        if (mb_stripos($question, $phrase) !== false || mb_stripos($keywords_field, $phrase) !== false) {
                            // FAQ ก็มี phrase นี้ด้วย → boost มาก (cap 2 phrases max)
                            $importantPhraseScore += 1000;
                            error_log("IMPORTANT PHRASE MATCH: '$phrase' found in both query and FAQ (Q: $question) +1000");
                        }
                    }
                }
                $score += $importantPhraseScore;
                
                // [+1500 pts] CRITICAL KEYWORD MATCH - คำสำคัญที่ต้องตอบตรงเรื่อง
                $criticalKeywords = ChatbotConfig::$criticalKeywords;
                ...
                // [ลบแล้ว] Double Exact Match +1000 ซ้ำกับ +2000 ด้านบน — เหลือ Exact Match +2000 อย่างเดียว
                
                // [500 pts] Phrase Match — ตรวจทุก alternative แล้วเลือก position ที่ดีที่สุด
                $bestPhrasePos = false;
                $bestLengthDiff = PHP_INT_MAX;
                foreach ($questionAlts as $alt) {
                    $pos = mb_stripos($alt, $query);
                    if ($pos !== false) {
                        if ($bestPhrasePos === false || $pos < $bestPhrasePos) {
                            $bestPhrasePos = $pos;
                        }
                    }
                    // หา alternative ที่ความยาวใกล้เคียง query ที่สุด
                    $diff = abs(mb_strlen($alt) - mb_strlen($query));
                    if ($diff < $bestLengthDiff) {
                        $bestLengthDiff = $diff;
                    }
                }
                
                if ($bestPhrasePos !== false) {
                    $score += 500;
                    
                    // [+300 pts] Position Bonus - คำค้นอยู่ต้นประโยค (ตำแหน่ง 0-2)
                    if ($bestPhrasePos <= 2) {
                        $score += 300;
                    } 
                    // [+100 pts] Position Bonus - ใกล้ต้นประโยค (ตำแหน่ง 3-8)
                    elseif ($bestPhrasePos <= 8) {
                        $score += 100;
                    }
                    
                    // [+400 pts] Length Match Bonus - เทียบกับ alternative ที่ใกล้เคียงที่สุด
                    if ($bestLengthDiff <= 5) {
                        $score += 400;
                    } elseif ($bestLengthDiff <= 15) {
                        $score += 200;
                    } elseif ($bestLengthDiff <= 30) {
                        $score += 50;
                    }
                }
                ...
                // ใช้ ChatbotConfig (single source of truth)
                $departments = ChatbotConfig::$departmentKeywords;
                ...
                // [-400 pts] General FAQ Penalty - ถ้าคำถามระบุสาขาชัดเจน แต่ FAQ เป็น general
                // ป้องกันไม่ให้ FAQ ทั่วไปชนะ FAQ เฉพาะสาขาเมื่อ user ถามเจาะจง
                if ($detectedDepartment && $row['department'] === 'general') {
                    $score -= 400;
                    error_log("General FAQ Penalty: query has dept '$detectedDepartment' but FAQ is general (Q: $question) -400");
                }
                
                // [-800 pts] หลักสูตร Category Penalty - FAQ หมวด "หลักสูตร" เป็น overview/PDF 
                // ถ้า user ไม่ได้ถามเจาะจงว่า "ดูรายละเอียดหลักสูตร" ให้ลดคะแนนลง
                if ($normalizedCategory === 'หลักสูตร') {
                    $wantsCurriculum = false;
                    $curriculumIndicators = ['รายละเอียด', 'ดูหลักสูตร', 'pdf', 'เอกสาร', 'มคอ', 'เล่มหลักสูตร', 'ดาวน์โหลด'];
                    foreach ($curriculumIndicators as $ci) {
                        if (mb_stripos($normalizedQuery, $ci) !== false) {
                            $wantsCurriculum = true;
                            break;
                        }
                    }
                    if (!$wantsCurriculum) {
                        $score -= 800;
                        error_log("Curriculum Penalty: FAQ cat=หลักสูตร but query lacks detail keywords (Q: $question) -800");
                    }
                }
                
                // [-800 pts] ต่อเนื่อง Penalty - FAQ ที่เป็น "ต่อเนื่อง/เทียบโอน"
                // ถ้า user ไม่ได้ถามเจาะจงว่า "ต่อเนื่อง" ให้ลดคะแนนลง
                $faqHasContinuing = (mb_stripos($question, 'ต่อเนื่อง') !== false || mb_stripos($keywords_field, 'ต่อเนื่อง') !== false);
                $queryWantsContinuing = (mb_stripos($normalizedQuery, 'ต่อเนื่อง') !== false || mb_stripos($normalizedQuery, 'เทียบโอน') !== false);
                if ($faqHasContinuing && !$queryWantsContinuing) {
                    $score -= 800;
                    error_log("Continuing Penalty: FAQ has 'ต่อเนื่อง' but query doesn't (Q: $question) -800");
                }
            ...
            // คืนค่า top results (เพิ่มเป็น 30 เพื่อให้ GenericHandler ดึงได้ครบทุกสาขา)
    ...
    private function detectCategory($query) {
        ...
        $categoryKeywords = [
            // tuition+fee+loan รวมเข้า ทุนการศึกษา (loan) — ตรวจก่อน หลักสูตร
    ...
    private function expandQuerySynonyms($query) {
        // ใช้ synonym จาก ChatbotConfig (Single Source of Truth)
        $synonyms = ChatbotConfig::$synonyms;
    ...
    private function normalizeQuery($query) {
        // ใช้ normalization จาก ChatbotConfig (Single Source of Truth)
        $normalizations = ChatbotConfig::$normalizations;
    ...
    private function extractKeywords($query) {
        ...
        // ===== ปรับปรุง: ALWAYS ลองตัดคำย่อยจาก commonWords =====
        // ภาษาไทยไม่มีช่องว่าง — คำเช่น "ติดเกรด" ต้องแยก "เกรด" ออกมาด้วย
        // เพื่อให้ SQL LIKE '%เกรด%' จับ FAQ ที่เกี่ยวข้องได้
        $commonWords = [
            ...
            // เพิ่ม: คำสำคัญที่ต้องแยกออกจากคำติดกัน
            'เกรด', 'พ้นสภาพ', 'รีไทร์', 'ลาพัก', 'ลาออก',
            'สหกิจศึกษา', 'สหกิจ', 'ฝึกงาน',
            'เปิดสอน', 'กี่ปี', 'กี่เทอม', 'ระยะเวลา',
            'คุณสมบัติ', 'ขั้นตอน', 'วิธี',
            'ทุนการศึกษา', 'ผลการเรียน',
            'ย้ายสาขา', 'โอนหน่วยกิต', 'เทียบโอน',
            // เพิ่ม: คำที่พบบ่อยในคำถามธรรมชาติ
            'ออกกำลังกาย', 'ชมรม', 'ตารางสอน', 'ตารางเรียน',
            'ตรงไหน', 'จุดเด่น', 'Retire', 'กู้เงิน',
            'สอบตก', 'สอบไม่ผ่าน', 'หลักสูตรต่อเนื่อง',
            // เพิ่ม: คำสำคัญสำหรับ admission/คุณสมบัติ
            'ต่างชาติ', 'ชาวต่างชาติ', 'ปวส', 'ปวช', 'วุฒิ', 'อัญมณี', 'เตรียมตัว',
            'วิศวะ', 'ม.6', 'ม.3', 'ต่อเนื่อง', 'รับสมัคร', 'สมัครเรียน', 'ย้าย',
            'มหาลัย', 'จบอะไร', 'รับจบ', 'นักศึกษาใหม่',
        ];
    ...
    private function searchNews($query) {
        ...
        $newsKeywords = ['ข่าว', 'ข่าวสาร', 'ประชาสัมพันธ์', 'กิจกรรม', 'อีเว้นท์', 'event', 'news', 'ล่าสุด'];
        $isNewsQuery = false;
        ...
        $sql .= " ORDER BY published_date DESC LIMIT 5";
    ...
    private function formatNewsList($newsList) {
        ...
        // Prepare structured news data for frontend rendering
        $newsData = [];
        foreach ($newsList as $news) {
            $item = [
                'title' => $news['title'],
                'category' => $news['category'],
                'date' => !empty($news['published_date']) ? date('d/m/Y', strtotime($news['published_date'])) : '',
                'summary' => '',
                'link' => $news['link_url'] ?? '',
                'thumb' => '',
            ];
            
            // Filter out bad thumbnails (emoji icons, loading gifs, tiny images)
            $thumbUrl = $news['thumbnail_url'] ?? '';
            if ($thumbUrl && !preg_match('/(emoji|ajax-loader|loading|favicon|icon)/i', $thumbUrl)) {
                $item['thumb'] = $thumbUrl;
            }
            
            if (!empty($news['summary']) && $news['summary'] !== $news['title']) {
                $summary = strip_tags($news['summary']);
                // Skip useless summaries
                $skipSummaries = ['รูปภาพทั้งหมด', 'Loading', '&hellip;'];
                $isUseful = true;
                foreach ($skipSummaries as $skip) {
                    if (mb_strlen($summary) < 20 || mb_stripos($summary, $skip) === 0) {
                        $isUseful = false;
                        break;
                    }
                }
                if ($isUseful) {
                    ...
                    $item['summary'] = mb_strlen($summary) > 120 
                        ? mb_substr($summary, 0, 120) . '...' 
                        : $summary;
                }
            }
        ...
        // Encode news data as JSON marker for frontend to parse and render as cards
        ...
        return "<!--NEWS_DATA:{$jsonData}:NEWS_DATA-->";
    }
    
    /**
     * ตรวจสอบว่า FAQ มีคำตอบ curated สำหรับคำถาม staff นี้หรือไม่
     * ใช้ก่อน return staff table เพื่อให้คำตอบจาก FAQ ที่ curated แล้วดีกว่า staff listing ทั้งสาขา
     * @return int|false FAQ ID ถ้าเจอ, false ถ้าไม่มี
     */
    private function hasCuratedStaffFaq($message) {
        try {
            $stmt = $this->db->prepare(
                "SELECT id FROM faq WHERE category = 'staff' AND is_active = 1 
            ...
            error_log("hasCuratedStaffFaq error: " . $e->getMessage());
            return false;
        }
    }
    ...
    private function searchStaff($query) {
        ...
        $departments = ChatbotConfig::$staffDepartmentMap;
        ...
        if ($targetDept) {
            ...
            // ตัดคำนำหน้าออกเพื่อค้นหาด้วยชื่อจริง
            $cleanName = preg_replace('/^(ผู้ช่วยศาสตราจารย์|รองศาสตราจารย์|ศาสตราจารย์|อาจารย์|ผศ\.ดร\.|รศ\.ดร\.|ผศ\.|รศ\.|ศ\.|ดร\.|อ\.ดร\.|อ\.)\s*/u', '', trim($query));
            $cleanName = trim($cleanName);
            error_log("searchStaff: query='$query', cleanName='$cleanName'");
            
            // Search all fields for specific name/position
            if (!empty($cleanName) && $cleanName !== $query) {
                // ค้นหาทั้งชื่อเต็มและชื่อที่ตัดคำนำหน้าแล้ว
                $sql .= " AND (name_th LIKE ? OR name_th LIKE ? OR name_en LIKE ? OR position_th LIKE ? OR expertise LIKE ?)";
                $params = ["%{$query}%", "%{$cleanName}%", "%{$cleanName}%", "%{$query}%", "%{$cleanName}%"];
            } else {
    ...
    /**
     * ค้นหาอาจารย์จากชื่อโดยตรง (ไม่ต้องมี keyword trigger)
     * ใช้เป็น last resort ก่อนตอบว่า "ไม่พบข้อมูล"
     */
    private function searchStaffByName($query) {
        $query = trim($query);
        
        // ตัดคำนำหน้าออก
        $cleanName = preg_replace('/^(ผู้ช่วยศาสตราจารย์|รองศาสตราจารย์|ศาสตราจารย์|อาจารย์|ผศ\.ดร\.|รศ\.ดร\.|ผศ\.|รศ\.|ศ\.|ดร\.|อ\.ดร\.|อ\.)\s*/u', '', $query);
        $cleanName = trim($cleanName);
        
        // ถ้าชื่อสั้นเกินไป (น้อยกว่า 3 ตัวอักษร) อาจจะไม่ใช่ชื่อคน
        if (mb_strlen($cleanName) < 3) {
            return [];
        }
        
        // ตัดคำที่ไม่ใช่ชื่อคนออก เช่น "อาจารย์สาขา..." หรือ "อาจารย์สอนวิชา..."
        $nonNameWords = ['สาขา', 'สอน', 'วิชา', 'ที่', 'คือ', 'มี', 'ใคร', 'ทั้งหมด', 'ภาค', 'คณะ', 'ห้อง', 'เบอร์', 'อีเมล', 'เงินเดือน'];
        foreach ($nonNameWords as $word) {
            if (mb_strpos($cleanName, $word) === 0) {
                return []; // ชื่อขึ้นต้นด้วยคำทั่วไป ไม่ใช่ชื่อคน
            }
        }
        
        error_log("searchStaffByName: query='$query', cleanName='$cleanName'");
        
        // ค้นหาจาก name_th ด้วยทั้งข้อความเดิมและชื่อที่ตัดคำนำหน้าแล้ว
        $sql = "SELECT * FROM staff WHERE is_active = 1 AND (name_th LIKE ? OR name_th LIKE ?)";
        $params = ["%{$query}%", "%{$cleanName}%"];
        
        // ถ้าชื่อมีช่องว่าง (ชื่อ นามสกุล) ลองแยกค้นหาทีละส่วน
        $nameParts = preg_split('/\s+/u', $cleanName);
        if (count($nameParts) >= 2) {
            // ค้นหาด้วยชื่อ + นามสกุลแยกกัน
            $sql .= " OR (name_th LIKE ? AND name_th LIKE ?)";
            $params[] = "%{$nameParts[0]}%";
            $params[] = "%{$nameParts[1]}%";
        }
        ...
        error_log("searchStaffByName: found " . count($results) . " results");
    ...
    private function buildStaffResponse($sessionId, $message, $staffList) {
        ...
        if (isset($staffList['_show_department_list']) && $staffList['_show_department_list']) {
            ...
            $answer .= "🔹 สาขาวิศวกรรมไฟฟ้าสื่อสารและระบบอัจฉริยะ\n";
