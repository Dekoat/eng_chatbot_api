...
class Chatbot {
    ...
    public function handleChat($sessionId, $message) {
        ...
        $forceNewsIndicators = ['ข่าว', 'ล่าสุด', 'ข่าวสาร', 'ประชาสัมพันธ์', 'อะไรบ้าง'];
        if ($skipNews) {
    ...
    private function searchFAQBroad($query, $filterCategory = null) {
        ...
        $intentPatterns = [
            ...
            // หมายเหตุ: ตัด 'อ.' ออกเพราะ match กับ 'กรอ.' ผิด (ทำให้ FAQ เกี่ยวกับกรอ.ถูกจัดเป็น staff intent)
        ...
        $departmentMap = [
            ...
            'สาขาสื่อสาร' => 'electronics_telecom_engineering',
        ...
        $skipDeptKeywords = ['แรงพอ', 'สเปค', 'ต่างจาก', 'เปรียบเทียบ', 'ข้อแตกต่าง', 'ต่างกัน', 'เหมือนกัน',
            // ถ้าพูดถึง "ปริญญาตรี" + "ปวช" แสดงว่าถามเรื่องการรับสมัคร ไม่ใช่สาขา vocational
            'ปริญญาตรี', 'สำหรับผู้จบ'];
        ...
        if ($filterCategory) {
            ...
            $relatedCategories = [
                'program' => ['curriculum', 'about', 'general', 'activities', 'contact', 'admission', 'หลักสูตร'],  // หลักสูตร↔รายวิชา↔เกี่ยวกับ↔กิจกรรม(สหกิจ)↔ติดต่อ↔การรับสมัคร
                'curriculum' => ['program', 'หลักสูตร'],
                'about' => ['program', 'general', 'contact', 'หลักสูตร'],
                'general' => ['about', 'document', 'cooperation', 'staff', 'facilities', 'research', 'activities', 'contact', 'หลักสูตร'],  // general=catch-all → AI มักจำแนกผิดเป็น general
                'loan' => ['admission'],               // ค่าเทอม↔การสมัคร
                'admission' => ['loan', 'program'],    // สมัคร↔ค่าเทอม↔หลักสูตร
                'contact' => ['activities', 'about', 'facilities', 'general'],  // AI สับสน contact↔activities↔เกี่ยวกับ↔สิ่งอำนวยความสะดวก
                'facilities' => ['contact', 'general'],  // สิ่งอำนวยความสะดวก↔ติดต่อ↔ทั่วไป
                'document' => ['cooperation'],          // เอกสาร↔ความร่วมมือ
                'cooperation' => ['document', 'about'],  // ความร่วมมือ↔เอกสาร↔เกี่ยวกับ
                'หลักสูตร' => ['program', 'curriculum', 'general', 'about'],  // หลักสูตร PDF → เกี่ยวข้องกับ program/curriculum
            ];
                ...
                foreach ($importantPhrases as $phrase) {
                    if (mb_stripos($query, $phrase) !== false) {
                        ...
                        if (mb_stripos($question, $phrase) !== false || mb_stripos($keywords_field, $phrase) !== false) {
                            // FAQ ก็มี phrase นี้ด้วย → boost มาก (สะสมได้หลาย phrase)
                            $score += 1000;
