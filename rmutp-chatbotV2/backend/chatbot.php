...
class AIHelper {
    ...
    public function mapIntentToCategory($intent) {
        // Map new AI intents to categories
        $mapping = [
...
class Chatbot {
    ...
    public function handleChat($sessionId, $message) {
        ...
        $staffKeywordsCheck = ['อาจารย์', 'ผศ', 'รศ', 'ดร.', 'หัวหน้าสาขา', 'คณาจารย์', 'บุคลากร', 'ใครสอน', 'อ.'];
    ...
    private function searchFAQBroad($query, $filterCategory = null) {
        ...
        $intentPatterns = [
            'location' => ['อยู่ที่ไหน', 'ตั้งอยู่', 'ที่ตั้ง', 'สถานที่จัดการเรียนการสอน', 'สถานที่เรียน', 'อาคาร', 'ชั้น', 'แผนที่', 'เดินทาง', 'MRT', 'BTS'],
            'contact' => ['ติดต่อ', 'เบอร์', 'โทร', 'อีเมล', 'email', 'โทรศัพท์', 'เว็บไซต์', 'Facebook', 'Line', 'ช่องทาง'],
            ...
            'facility' => ['ห้องปฏิบัติการ', 'ห้องแล็บ', 'lab', 'อุปกรณ์', 'สิ่งอำนวยความสะดวก', 'ห้องเรียน', 'เครื่องมือ'],
            'activity' => ['กิจกรรม', 'โครงการ', 'งานวิจัย', 'research', 'ฝึกงาน', 'สหกิจ', 'อบรม'],
            ...
            'tuition' => ['ค่าเทอม', 'ค่าเล่าเรียน', 'ค่าใช้จ่าย', 'ค่าลงทะเบียน', 'เงิน', 'ราคา', 'เท่าไหร่', 'กี่บาท'],
            'scholarship' => ['ทุน', 'กยศ', 'กรอ', 'ทุนการศึกษา', 'กู้ยืม', 'เงินกู้', 'ทุนเรียนดี', 'scholarship'],
        ...
        $departmentMap = [
            ...
            'สื่อสารและระบบอัจฉริยะ' => 'sime_engineering',
            'ปวช' => 'vocational',
        ...
        $params = [];
        if ($filterCategory) {
            // รวม FAQ จาก general department เสมอเพื่อให้คำตอบทั่วไปมีโอกาสปรากฏ
            $sql .= "AND (f.category = ? OR f.department = 'general') ";
            $params[] = $filterCategory;
            error_log("Filtering FAQ by category: $filterCategory (including general dept)");
        }
        
        // เพิ่ม department filter ถ้าตรวจพบชื่อสาขาชัดเจน
        if ($detectedDepartment) {
            $sql .= "AND f.department = ? ";
            $params[] = $detectedDepartment;
            error_log("Filtering FAQ by department: $detectedDepartment");
        }
        ...
        try {
            ...
            foreach ($results as &$row) {
                ...
                // [+2000 pts] EXACT MATCH - สูงสุด!
                if (mb_strtolower(trim($question)) === mb_strtolower(trim($query))) {
                    $score += 2000;
                    error_log("EXACT MATCH FOUND! Q: $question (+2000)");
                }
    ...
    private function detectCategory($query) {
        $categoryKeywords = [
            ...
            'ทุนการศึกษา' => ['ทุน', 'กยศ', 'กรอ', 'ทุนการศึกษา', 'กู้ยืม', 'ทุนกู้', 'scholarship', 'มีทุนอะไรบ้าง', 'มีทุนไหม'],
            'สิ่งอำนวยความสะดวก' => ['หอพัก', 'WiFi', 'ห้องสมุด', 'โรงอาหาร', 'ATM', 'ที่พัก', 'dormitory', 'library'],
