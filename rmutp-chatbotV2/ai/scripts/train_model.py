...
# Add current directory to path for text_utils import
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from text_utils import tokenize_thai  # import จากไฟล์แยก
...
class IntentClassifier:
    def __init__(self, model_type='logistic_regression'):
        ...
        if model_type == 'naive_bayes':
            ...
            # LogisticRegression with C=10 gives best cross-validation score (~95.8%)
            self.model = LogisticRegression(max_iter=1000, C=10, random_state=42)
    ...
    def load_data(self, filepath='../data/training_data.csv'):
        ...
        # ใช้ column 'text' แทน 'question'
        text_column = 'text' if 'text' in df.columns else 'question'
        return df[text_column].values, df['intent'].values
    ...
    def train(self, X, y, test_size=0.2, random_state=42):
        ...
        if test_size > 0:
            ...
            X_test = X[:50]
            y_test = y[:50]
            print(f"\nUsing ALL {len(X_train)} examples for training (production mode)")
            print(f"Sanity check with first 50 examples")
        ...
        # Train model
        model_name = "Logistic Regression (C=10)" if self.model_type == 'logistic_regression' else "Naive Bayes"
    ...
    def save_model(self, model_dir='../models'):
        """Save trained model and vectorizer (แยก components)"""
        # Handle both relative and absolute paths
        ...
        # บันทึกแยก component ไม่ใช่ทั้ง object
        import pickle
        with open(model_path, 'wb') as f:
            # บันทึกเฉพาะ sklearn model
            pickle.dump(self.model, f, protocol=4)
        with open(vectorizer_path, 'wb') as f:
            # บันทึกเฉพาะ vectorizer
