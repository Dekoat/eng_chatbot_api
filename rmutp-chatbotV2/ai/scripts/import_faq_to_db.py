...
class FAQImporter:
    ...
    def insert_faq(self, faq_data):
        ...
        # Insert ข้อมูล
        query = """
        INSERT INTO faq (question, answer, category, department, keywords, created_at, updated_at)
        VALUES (%s, %s, %s, %s, %s, NOW(), NOW())
        """
        
        # แปลง keywords เป็น string
        keywords_str = faq_data.get('keywords', '') if isinstance(faq_data.get('keywords'), str) else ''
        
        values = (
            faq_data['question'],
            faq_data['answer'],
            faq_data.get('category', 'general'),
            faq_data.get('department', 'general'),
            keywords_str
        )
...
def main():
    ...
    # Import จากไฟล์ (รับจาก argument หรือใช้ default)
    if len(sys.argv) > 1:
        json_file = sys.argv[1]
    else:
        json_file = "ai/data/faq_person4_sime_history_20260125.json"
